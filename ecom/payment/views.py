...
def process_order(request):
	if request.POST:
		...
		payement_form = PaymentForm(request.POST or None)
		...
		if request.user.is_authenticated:
			...
			create_order =Oder(full_name=full_name, email=email,shipping_address=shipping_address,amount_paid=amount_paid)
			create_order.save()	
...
def billing_info(request):
	if request.POST:
		...
		return render(request, "payment/billing_info.html", {"cart_products":cart_products, "quantites":quantites, "totals":totals, "shipping_form":shipping_form,"billing_form":billing_form})	
	else:
		messages.success(request, "Access Denied")
		return redirect('home')
